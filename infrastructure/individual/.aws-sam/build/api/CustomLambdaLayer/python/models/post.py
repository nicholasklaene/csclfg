...
class CreatePostRequest(IRequest):
    def __init__(self, title: str, preview: str, description: str, category: str, tags: List[str]):
        self.title = title
    ...
    def validate(self):
        errors = []
        if len(self.title) == 0 or len(self.title) > 128:
            errors.append("Title must be between 0 and 128 characters")
        if len(self.preview) == 0 or len(self.preview) > 256:
            errors.append("Preview must be between 0 and 256 characters")
